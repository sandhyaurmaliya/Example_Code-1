    ...
    var gebruiker_token = JSON.parse(sessionStorage.getItem('gebruiker_token'))
        ...
        var authorization = {
          ...
          .done(function (response) {
            ...
            else if(document.body.className == 'producten_beheren_page')
            {
...
function bevestig(actie,id){
    gebruiker_token = JSON.parse(sessionStorage.getItem('gebruiker_token'))
    var klant_status;
      ...
      if(actie == 'verwijderen'){
          ...
          $.ajax({
             ...
             var error_msg = response.responseJSON.status.error_code;           
              ...
              if (error_msg == 23000)
              {
                console.log("deze klant heeft relatie constraint");
              }
...
function toon_modal(actie, klantID){
    ...
    if(actie =='toevoegen'){
      document.getElementById('klantModalLabel').innerHTML = "Product Toevoegen"
    ...
    if (actie =='bewerken'){
        ...
        document.getElementById('klantModalLabel').innerHTML = "Product Bewerken"
...
function filter_waarde()
{
  ...
  $.ajax({   
    ...
    data: {         
        ...
        "filter": ["CONCAT(id, ' ',email, ' ', voornaam, ' ', adres)", "LIKE", "%"+zoekwaarde+"%"],
